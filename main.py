...
def exploratory_analysis_histogram_target(dataset,namesDataset):
    ...
    if isinstance(namesDataset, list) and len(namesDataset) > 1:
        ...
        for i, ax in enumerate(axes.flat):
            ...
            if i == 4*3-1:
                plt.savefig("figures/TargetFeatureHistogramPerDataset")
                ...
                plt.savefig("figures/TargetFeatureHistogramWholeDataset")
    else:
        plt.Figure(figsize=(5,5))
        sns.histplot(data=dataset, x='target', y=None, hue=None)
        plt.xlabel('Inhibition Constant ($K_i$) Values')
        plt.ylabel('Count')
        plt.title("Histogram of target feature for the {namesDataset} dataset")
        plt.savefig(f"figures/TargetFeatureHistogram_{namesDataset}")
...
def main():
    ...
    if choice == 1:
        ...
        print("\n --Performing a brief exploratory analysis, figures saved in folder figures")
        exploratory_analysis_histogram_target(dfGeneral, nameDataset)
        ...
        for i, smile in tqdm(enumerate(dfGeneral['smiles']), total=len(dfGeneral), leave=True, desc="Computing ECT"):
            g = from_smiles(smile)
            g.x = g.x.float()
            ect = compute_ect_edges(
                ...
                scale=500)
            g.ect=ect
            y = torch.tensor(dfGeneral['target'][i], dtype=torch.float).view(1, -1)
            g.y = y
            graph_list.append(g)
        ...
        try:
            os.stat(f"results/{SelectedDataset}/")
        except:
            os.mkdir(f"results/{SelectedDataset}/")

        df_results.to_csv(f'results/ADRA1A/ECT.csv', index=False)
        ...
        for enum, fingerprint in enumerate(fingerprint_list):
            ...
            dfsFingerprint=[]
            for name in nameDataset:
                dfF = pd.read_pickle(f"data/features/{name}/{name}_{fingerprint}.pkl")
                dfF = dfF.drop_duplicates(subset=['smiles'], keep='first')
                dfF = dfF.drop("target", axis=1)
                dfF["dataset"]=name
                dfsFingerprint.append(dfF)
            dfFTotal = pd.concat(dfFingerprint,ignore_index=True)
            df = pd.merge(dfGeneral, dfFTotal, how="left", on="smiles")
            ...
            df_results.to_csv(f'results/ADRA1A/{fingerprint}.csv', index=False)
            print(f"Done for fingerprint {fingerprint}")
        print("Done for all fingerprint methods")

        for enum, descriptor in enumerate(descriptor_list):
            print(f"\n Method: Descriptor {descriptor}")
            descriptor_path = f"data/features/{SelectedDataset}"
            dfDescriptor = pd.read_pickle(os.path.join(descriptor_path,f"{SelectedDataset}_{descriptor}.pkl"))
            ...
            df_results.to_csv(f'results/ADRA1A/{descriptor}.csv', index=False)
            print(f"Done for descriptor {descriptor}")
        print("Done for all descriptors methods")
        ...
        for enum, gnn in enumerate(gnns_list):
            ...
            df_results = pd.concat([results_rmse, results_r2, results_mae], ignore_index=True)
            df_results.to_csv(f'results/ADRA1A/{gnn}.csv', index=False)
            print(f"Done for Graph Neural Network {gnn}")
        print("Done for all GNNs methods")
        ...
        file_path = f"data/datasetRaw/{SelectedDataset}.sdf"
        df = load_molecule_dataset(file_path, SelectedDataset)
        df = remove_duplicates_and_nans(df)
        save_molecule_dataset_as_csv(df, SelectedDataset)
        print(f"Dataset {SelectedDataset} saved as CSV at data/datasetCSV/{SelectedDataset}.csv .")

        dfDataset = pd.read_csv(os.path.join(datasetPath,f"{SelectedDataset}.csv"))
        print("\n --Performing a brief exploratory analysis, figures saved in folder figures")
        exploratory_analysis_histogram_target(df, SelectedDataset)
        ...
        for enum, fingerprint in enumerate(fingerprint_list):
            print(f"\n Method: Fingerprint {fingerprint}")
            fingerprint_path = f"data/features/{SelectedDataset}"
            dfFingerprint = pd.read_pickle(os.path.join(fingerprint_path,f"{SelectedDataset}_{fingerprint}.pkl"))
